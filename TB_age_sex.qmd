...
```{r}
TB_age_sex |>
  filter(country_name == "Denmark", sex == "Both")
```
...
TB_age_sex |>
  filter(country_name == "Denmark", risk_factor != "all") |> #The ones that are not labelled "all". 
  summarise(sum_TB = sum(TB_cases_best))
```

```{r}
TB_age_sex |>
  filter(country_name == "Denmark", risk_factor == "all") |> #The ones that ARE labelled "all". 
  summarise(sum_TB = sum(TB_cases_best))
```

'Risk factor = all' is not specified anywhere in the WHO report.

Therefore, based on the analysis above, it seems safe to draw a conclusion on the data above, that 'risk factor = all' actually means 'risk factor = no risk factor'.
