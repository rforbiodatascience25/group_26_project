...
TB_age_sex <- TB_age_sex |>
  ...
  mutate( 
        TB_cases_pr_100k_best = TB_cases_best/10^5,
        TB_cases_pr_100k_min = TB_cases_min/10^5,
        TB_cases_pr_100k_max = TB_cases_max/10^5,
        
        #For each each country, calculate the summed pr. 100k amount of cases
        total_TB_cases_pr_100k_best = total_TB_cases_best/10^5,
        total_TB_cases_pr_100k_min = total_TB_cases_min/10^5, 
        total_TB_cases_pr_100k_max = total_TB_cases_max/10^5)
...
Average amount of TB cases pr. 100k citizens, worldwide:

**3.97 +/- 16.34 TB cases pr. 100k**
...
```{r}
#Making an object for storing the top 10 countries with most TB cases pr. 100k: 
...
```{r}
ggplot(TB_top_summary, aes(x = mean_best, y = country)) +
...
**Scatter plot with error bars - Countries with above 2 TB case pr. 100k:**
...
The strak contrast between infection and low-infection countries skews the boxplots. It is therefore important to focus more concretely on some of the high-infection countries.
...
```{r}
#Scatter plot for TB cases for different sexes in top 10 most infected countries
...
There is not really statistical significance difference between male and females. Males tend to be sightly more infected.
...
```{r}
#Removing 15+, 18+ and 'all' from the barplot: 

TB_age_sex |>
  filter(!age_group %in% c("15+", "18+", "all")) |> #Without these 3 groups. 
  group_by(age_group) |>
  summarise(total_TB = sum(TB_cases_pr_100k_best, na.rm = TRUE)) |>
  mutate(
    age_lower = as.numeric(str_extract(age_group, "^[0-9]+")),
...
TB_age_sex |>
  ...
  filter(country %in% top_10_countries) |> #Getting only the top 10 most infected countries
  group_by(age_group) |>
  summarise(
    total_best = sum(TB_cases_pr_100k_best, na.rm = TRUE),
    total_min  = sum(TB_cases_pr_100k_min,  na.rm = TRUE),
...
TB_age_sex |>
  ...
  summarise(total_TB_cases_worldwide = sum(total_TB_cases)) |> #71744072 total TB cases
  summarise(normalized = total_TB_cases_worldwide/(sum(TB_age_sex$population_size))*10^5)
