...
```{r}
#Scatterplot - Sexes and TB cases:
...
```{r}
ggplot(TB_summary, aes(x = summed_TB_cases_pr_100k_best, y = country)) +
  geom_point(size = 3, color = "orange") +
  geom_errorbarh(
    aes(xmin = summed_TB_cases_pr_100k_min, 
        xmax = summed_TB_cases_pr_100k_max), height = 0.2) +
  labs(
    x = "TB cases per 100k\n(Best estimate with min/max)",
    y = "Country",
    title = "Top 10 Countries — TB Cases per 100k with error bars"
...
ggplot(TB_summary, aes(x = summed_TB_cases_pr_100k_best, y = country)) +
  ...
  labs(
    x = "TB cases per 100k\n(Best estimate with min/max)",
    y = "Country",
    title = "Top 10 Countries — TB Cases per 100k by Sex"
  ) +
  facet_wrap(~ sex) +
  theme_minimal()
```
