...
**Loading data:** (Erase this block after finishing and trying everything)
...
Get only the data dictionary based on burden countries (and some extra) to understand the data on TB_burden and be able to preoperly clean it:
...
For the cases with only 25 cases we can see it's because of Namibia not having an iso2 country code, and also 
for North Korea not providing enough information.
For cases like e_tbhiv_prct_lo and e_tbhiv_prct_hi we see that the whole column is filled with NA's.
And finally there are some other columns with NA's on some specific countries.
...
Now we save it into our data folder which is already cleaned:
